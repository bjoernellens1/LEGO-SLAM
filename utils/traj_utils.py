...
class TrajManager:
    def __init__(self, which_dataset, dataset_path):
        ...
        Args:
        which_dataset : "tum" or "replica" or "scannet"
        ...
        if self.which_dataset == "tum":
            ...
            self.gt_poses = self.scannet_load_poses(os.path.join(self.dataset_path, 'pose'))
        elif self.which_dataset == "femto":
            # Femto doesn't have GT poses, count images to initialize pose array
            ...
            if os.path.exists(images_folder):
                ...
                # Create identity matrices for each image (will be replaced by SLAM estimates)
                self.gt_poses = [np.eye(4) for _ in range(num_images)]
                print(f"Femto dataset: Initialized {num_images} poses (no GT available)")
            else:
                self.gt_poses = [np.eye(4)]
                print("Warning: images folder not found, using single pose")
        else:
    ...
    def scannet_load_poses(self, path):
        """
        Load poses from ScanNet dataset
        Each pose is stored in a separate .txt file with 4x4 matrix format
        """
        import glob
        poses = []
